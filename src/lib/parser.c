...
parser_error_t parser_validate(parser_context_t *ctx) {
    ...
    err = crypto_validate_context(parser_tx_obj.oasis.oasis_tx.method);
...
__Z_INLINE parser_error_t parser_getType(parser_context_t *ctx, char *outVal, uint16_t outValLen) {
    switch (parser_tx_obj.oasis.oasis_tx.method) {
    ...
    switch (parser_tx_obj.oasis.oasis_tx.method) {
        case stakingTransfer:
            switch (displayDynamicIdx) {
                case 0: {
                    ...
                    return parser_printPublicKey(&parser_tx_obj.oasis.oasis_tx.body.stakingTransfer.xfer_to,
                                                 outVal, outValLen, pageIdx, pageCount);
                }
                case 1: {
                    snprintf(outKey, outKeyLen, "Tokens");
                    return parser_printQuantity(&parser_tx_obj.oasis.oasis_tx.body.stakingTransfer.xfer_tokens,
                                                outVal, outValLen, pageIdx, pageCount);
                }
            }
            break;
        case stakingBurn:
            switch (displayDynamicIdx) {
                case 0: {
                    snprintf(outKey, outKeyLen, "Tokens");
                    return parser_printQuantity(&parser_tx_obj.oasis.oasis_tx.body.stakingBurn.burn_tokens,
                                                outVal, outValLen, pageIdx, pageCount);
                }
            }
            break;
        case stakingAddEscrow:
            switch (displayDynamicIdx) {
                case 0: {
                    ...
                    return parser_printPublicKey(&parser_tx_obj.oasis.oasis_tx.body.stakingAddEscrow.escrow_account,
                                                 outVal, outValLen, pageIdx, pageCount);
                }
                case 1: {
                    snprintf(outKey, outKeyLen, "Tokens");
                    return parser_printQuantity(&parser_tx_obj.oasis.oasis_tx.body.stakingAddEscrow.escrow_tokens,
                                                outVal, outValLen, pageIdx, pageCount);
                }
            }
            break;
        case stakingReclaimEscrow:
            switch (displayDynamicIdx) {
                case 0: {
                    snprintf(outKey, outKeyLen, "Escrow");
                    return parser_printPublicKey(&parser_tx_obj.oasis.oasis_tx.body.stakingReclaimEscrow.escrow_account,
                                                 outVal, outValLen, pageIdx, pageCount);
                }
                case 1: {
                    snprintf(outKey, outKeyLen, "Tokens");
                    return parser_printQuantity(&parser_tx_obj.oasis.oasis_tx.body.stakingReclaimEscrow.reclaim_shares,
                                                outVal, outValLen, pageIdx, pageCount);
                }
            }
            break;
        case stakingAmendCommissionSchedule:
            if (displayDynamicIdx / 2 < (int) parser_tx_obj.oasis.oasis_tx.body.stakingAmendCommissionSchedule.rates_length) {
                ...
                switch (displayDynamicIdx % 2) {
                    case 0: {
                        ...
                        uint64_to_str(outVal, outValLen,
                                      parser_tx_obj.oasis.oasis_tx.body.stakingAmendCommissionSchedule.rate.start);
                        return parser_ok;
                    }
                    case 1: {
                        snprintf(outKey, outKeyLen, "Rates : [%i] rate", index);
                        return parser_printRate(&parser_tx_obj.oasis.oasis_tx.body.stakingAmendCommissionSchedule.rate.rate,
                                                outVal, outValLen, pageIdx, pageCount);
                    }
                }
            } else {
                const int8_t index = (displayDynamicIdx -
                                      parser_tx_obj.oasis.oasis_tx.body.stakingAmendCommissionSchedule.rates_length * 2) / 3;

                // Need to do it once for each bound which is every 3 displayIdx
                if ((displayDynamicIdx - parser_tx_obj.oasis.oasis_tx.body.stakingAmendCommissionSchedule.rates_length * 2) %
                    3 == 0) {
                    // Only keeping one amendment in body at the time
                ...
                switch ((displayDynamicIdx -
                    ...
                    case 0: {
                        ...
                        uint64_to_str(outVal, outValLen,
                                      parser_tx_obj.oasis.oasis_tx.body.stakingAmendCommissionSchedule.bound.start);
                        return parser_ok;
                    }
                    case 1: {
                        snprintf(outKey, outKeyLen, "Bounds : [%i] min", index);
                        return parser_printRate(
                                &parser_tx_obj.oasis.oasis_tx.body.stakingAmendCommissionSchedule.bound.rate_min,
                                outVal, outValLen, pageIdx, pageCount);
                    }
                    case 2: {
                        snprintf(outKey, outKeyLen, "Bounds : [%i] max", index);
                        return parser_printRate(
                                &parser_tx_obj.oasis.oasis_tx.body.stakingAmendCommissionSchedule.bound.rate_max,
                                outVal, outValLen, pageIdx, pageCount);
                    }
                }
            }
        ...
        case registryUnfreezeNode:
            if (displayDynamicIdx == 0) {
                ...
                return parser_printPublicKey(&parser_tx_obj.oasis.oasis_tx.body.registryUnfreezeNode.node_id,
                                             outVal, outValLen, pageIdx, pageCount);
            }
        case unknownMethod:
        default:
...
__Z_INLINE parser_error_t parser_getItemTx(parser_context_t *ctx,
    ...
    if (displayIdx == 1) {
        ...
        return parser_printQuantity(&parser_tx_obj.oasis.oasis_tx.fee_amount, outVal, outValLen, pageIdx, pageCount);
    }

    if (displayIdx == 2) {
        snprintf(outKey, outKeyLen, "Fee Gas");
        uint64_to_str(outVal, outValLen, parser_tx_obj.oasis.oasis_tx.fee_gas);
        return parser_ok;
    }
    ...
    if (parser_customContextEnabled()) {
        ...
        if (displayIdx == 3) {
            ...
            pageString(outVal, outValLen,
                       (char *) crypto_get_context_suffix(parser_tx_obj.oasis.oasis_tx.method),
                       pageIdx, pageCount);
            return parser_ok;
        }
    }
...
__Z_INLINE parser_error_t parser_getItemEntity(parser_context_t *ctx,
    ...
    if (displayIdx == 0) {
        ...
        return parser_printPublicKey(&parser_tx_obj.oasis.oasis_entity.id,
                                     outVal, outValLen, pageIdx, pageCount);
    }

    if (displayIdx <= (int) parser_tx_obj.oasis.oasis_entity.nodes_length) {
        ...
        return parser_printPublicKey(&parser_tx_obj.oasis.oasis_entity.node,
                                     outVal, outValLen, pageIdx, pageCount);
    }

    if (displayIdx - parser_tx_obj.oasis.oasis_entity.nodes_length == 1) {
        if (parser_tx_obj.oasis.oasis_entity.allow_entity_signed_nodes) {
