...
__Z_INLINE parser_error_t parser_getDynamicItem(parser_context_t *ctx,
    ...
    switch (parser_tx_obj.oasis_tx.method) {
        ...
        case stakingReclaimEscrow:
            ...
            break;
        case stakingAmendCommissionSchedule:
            if (displayDynamicIdx / 2 < (int) parser_tx_obj.oasis_tx.body.stakingAmendCommissionSchedule.rates_length) {
                ...
                switch (displayDynamicIdx % 2) {
                    case 0: {
                        ...
                        uint64_to_str(outVal, outValLen,
                                      parser_tx_obj.oasis_tx.body.stakingAmendCommissionSchedule.rate.start);
                        return parser_ok;
                    }
                    case 1: {
                ...
                const int8_t index = (displayDynamicIdx -
                                      parser_tx_obj.oasis_tx.body.stakingAmendCommissionSchedule.rates_length * 2) / 3;

                // Need to do it once for each bound which is every 3 displayIdx
                if ((displayDynamicIdx - parser_tx_obj.oasis_tx.body.stakingAmendCommissionSchedule.rates_length * 2) %
                    3 == 0) {
                    // Only keeping one amendment in body at the time
                ...
                switch ((displayDynamicIdx -
                    ...
                    case 0: {
                        ...
                        uint64_to_str(outVal, outValLen,
                                      parser_tx_obj.oasis_tx.body.stakingAmendCommissionSchedule.bound.start);
                        return parser_ok;
                    }
                    case 1: {
                        snprintf(outKey, outKeyLen, "Bounds : [%i] min", index);
                        return parser_printRate(
                                &parser_tx_obj.oasis_tx.body.stakingAmendCommissionSchedule.bound.rate_min,
                                outVal, outValLen, pageIdx, pageCount);
                    }
                    case 2: {
                        snprintf(outKey, outKeyLen, "Bounds : [%i] max", index);
                        return parser_printRate(
                                &parser_tx_obj.oasis_tx.body.stakingAmendCommissionSchedule.bound.rate_max,
                                outVal, outValLen, pageIdx, pageCount);
                    }
                }
            }
